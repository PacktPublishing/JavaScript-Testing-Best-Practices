...
describe("Story: Invalid path ", () => {
    ...
    it ("should throw an error when given a number", async() => {
        try {
            await resizer(1);
            assert.fail("Should have failed");
        }
            ...
            expect(err).to.be.ok;
            expect(err.message).to.equal("Source must be a string");
        }
    });

    it("should throw an error when given an invalid file type", async() => {
        try {
            await resizer("whatever.pdf", "whateve1.pdf");
            assert.fail("Should have failed");
        }
        catch(err) {
            expect(err).to.be.ok;
